...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^kdp_") ; then
       CM_BUILD=$(echo -n $1 | sed -e 's/^kdp_//g')
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/kdp/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
function eat()
{
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.kdp.device=$CM_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.kdp.device=$CM_BUILD") || [ "$FORCE_PUSH" == "true" ];
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/kdp/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
